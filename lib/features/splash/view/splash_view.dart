...
//* */======== ADDED CUSTOM TIMER FOR SPLASH SCREEN ===================//
...
class _SplashViewState extends ConsumerState<SplashView> {
  ...
  ProviderContainer? _containerData;
  ...
  void _checkIfReadyToComplete() {
    ...
    if (_minimumDurationCompleted && _initializationCompleted && _containerData != null) {
      talker.info("Initialization takes ${stopwatch.elapsedMilliseconds}");
      widget.onInitialized(_containerData!);
    }
  }
  ...
  Widget build(BuildContext context) {
    return Consumer(
      builder: (context, ref, child) {
        ...
        ref.listen(
          ...
          (previous, next) {
            if (next is AsyncData && next.valueOrNull != null) {
              ...
              _containerData = next.requireValue;
              _checkIfReadyToComplete();
            }
          },
        );
        return futureAsync.easyWhen(
          data: (data) {
            // Still show the splash even when data is ready
            // until minimum duration is also completed
            return _minimumDurationCompleted ? const SizedBox.shrink() : child!;
          },
          loadingWidget: () => child!,
          errorWidget: (error, stackTrace) => child!,
        );
